"""Tests for replaceable scope support (on_reentry="replace")."""
...
class TestReplaceableScopeBasic:
    ...
    def test_replace_creates_new_context(self, container: Container) -> None:
        ...
        with container.scoped_context("task") as ctx_outer:
            with container.scoped_context("task", on_reentry="replace") as ctx_inner:
    ...
    def test_replace_has_own_instance_cache(self, container: Container) -> None:
        ...
        with container.scoped_context("task"):
            ...
            with container.scoped_context("task", on_reentry="replace"):
    ...
    def test_replace_three_levels(self, container: Container) -> None:
        ...
        with container.scoped_context("task") as ctx1:
            ...
            with container.scoped_context("task", on_reentry="replace") as ctx2:
                ctx2.set(ScopedValue, ScopedValue("level-2"))
                assert container.resolve(ScopedValue).label == "level-2"

                with container.scoped_context("task", on_reentry="replace") as ctx3:
...
class TestReplaceableScopeAsync:
    ...
    async def test_async_replace_has_own_cache(self, container: Container) -> None:
        ...
        async with container.ascoped_context("task") as ctx1:
            ...
            async with container.ascoped_context("task", on_reentry="replace") as ctx2:
    ...
    async def test_async_replace_resource_cleanup(self, container: Container) -> None:
        ...
        async with container.ascoped_context("task"):
            ...
            async with container.ascoped_context("task", on_reentry="replace"):
                await container.aresolve(str)
...
class TestReentryAssertion:
    """Tests for the reentry assertion axis: allow (default) / forbid / require.

    Covers whether an entry may or must be a re-entry of an already-active
    same-scope context, independent of what on_reentry then does with it.
    """

    # -- allow (default): first entry or re-entry both fine --

    def test_reentry_reuses_context_by_default(self, container: Container) -> None:
        """Defaults (reentry='allow', on_reentry='reuse') reuse the context."""
    ...
    # -- forbid: this entry must be the root (no active context) --

    def test_forbid_allows_first_entry(self, container: Container) -> None:
        """reentry='forbid' succeeds on a first entry (it is the root)."""
        container.register_scope("task")

        with container.scoped_context("task", reentry="forbid") as ctx:
            assert container.get_scoped_context("task") is ctx

    async def test_async_forbid_allows_first_entry(self, container: Container) -> None:
        """Async reentry='forbid' succeeds on a first entry."""
        container.register_scope("task")

        async with container.ascoped_context("task", reentry="forbid") as ctx:
            assert container.get_scoped_context("task") is ctx
    ...
    # -- require: this entry must be nested in an active context --

    def test_require_raises_on_first_entry(self, container: Container) -> None:
        """reentry='require' raises when no same-scope context is active."""
        container.register_scope("task")

        with pytest.raises(
            RuntimeError,
            match=r"requires an already-active context",
        ):
            with container.scoped_context("task", reentry="require"):
                pass

    async def test_async_require_raises_on_first_entry(
        self, container: Container
    ) -> None:
        """Async reentry='require' raises when no context is active."""
        container.register_scope("task")
        ...
        with pytest.raises(
            ...
            async with container.ascoped_context("task", reentry="require"):
                pass

    def test_require_reuses_active_context(self, container: Container) -> None:
        """reentry='require' with default on_reentry reuses the active context."""
        container.register_scope("task")

        with container.scoped_context("task") as ctx_outer:
            with container.scoped_context("task", reentry="require") as ctx_inner:
                assert ctx_inner is ctx_outer

    async def test_async_require_reuses_active_context(
        self, container: Container
    ) -> None:
        """Async reentry='require' reuses the active context by default."""
        container.register_scope("task")

        async with container.ascoped_context("task") as ctx_outer:
            async with container.ascoped_context(
                "task", reentry="require"
            ) as ctx_inner:
                assert ctx_inner is ctx_outer

    def test_require_with_replace_isolates(self, container: Container) -> None:
        """reentry='require' + on_reentry='replace' opens a fresh isolated child."""
        container.register_scope("task", replaceable=True)

        with container.scoped_context("task") as ctx_outer:
            with container.scoped_context(
                "task", reentry="require", on_reentry="replace"
            ) as ctx_inner:
                assert ctx_inner is not ctx_outer
            assert container.get_scoped_context("task") is ctx_outer

    async def test_async_require_with_replace_isolates(
        self, container: Container
    ) -> None:
        """Async reentry='require' + on_reentry='replace' isolates a fresh child."""
        container.register_scope("task", replaceable=True)
...
class TestOnReentryBehavior:
    """Tests for the on_reentry behaviour axis: reuse (default) / replace.

    Covers the replaceable opt-in guard and the fact that on_reentry is
    ignored on a first entry. The replace *mechanics* (isolation, caching,
    cleanup) live in TestReplaceableScopeBasic / TestReplaceableScopeAsync.
    """

    def test_replace_raises_on_non_replaceable_scope(
        self, container: Container
    ) -> None:
        """on_reentry='replace' on a non-replaceable active scope raises."""
        container.register_scope("task")
    ...
    async def test_async_replace_raises_on_non_replaceable_scope(
        ...
        """Async on_reentry='replace' on a non-replaceable active scope raises."""
        container.register_scope("task")
    ...
    def test_replace_on_first_entry_does_not_require_optin(
        ...
        """on_reentry='replace' on a first entry just creates a context."""
        container.register_scope("task")

        with container.scoped_context("task", on_reentry="replace") as ctx:
            assert container.get_scoped_context("task") is ctx
...
class TestReplaceableScopeWithBuild:
    ...
    def test_build_then_replace_resolution(self, container: Container) -> None:
        ...
        with container:
            with container.scoped_context("task") as ctx_a:
                ...
                with container.scoped_context("task", on_reentry="replace") as ctx_b:
    ...
    async def test_build_then_async_replace_resolution(
        ...
        async with container.ascoped_context("task") as ctx_a:
            ...
            async with container.ascoped_context("task", on_reentry="replace") as ctx_b:
                ctx_b.set(ScopedValue, ScopedValue("inner"))
...
class TestReplaceableScopeParallelAsync:
    ...
    async def test_parallel_gather_with_parent_scope(
        ...
        async def run_task(label: str) -> str:
            async with container.ascoped_context("task", on_reentry="replace") as ctx:
                ctx.set(ScopedValue, ScopedValue(label))
                await asyncio.sleep(0)
    ...
    async def test_parallel_create_task_require_sees_parent(
        self, container: Container
    ) -> None:
        """reentry='require' passes across a create_task boundary.

        create_task copies the current context, so a scope active in the parent
        is still active inside the child task. A child that asserts reentry='require'
        must therefore succeed (not raise), and on_reentry='replace' still gives it
        an isolated context that leaves the parent untouched.
        """
        container.register_scope("task", replaceable=True)
        container.register(ScopedValue, scope="task", from_context=True)
        ...
        async def run_task(label: str) -> None:
            async with container.ascoped_context(
                ...
                ctx.set(ScopedValue, ScopedValue(label))
                await asyncio.sleep(0)
                results[label] = (await container.aresolve(ScopedValue)).label
    ...
    async def test_parallel_gather_require_sees_parent(
        self, container: Container
    ) -> None:
        """reentry='require' + replace isolates across gather under a parent."""
        container.register_scope("task", replaceable=True)
        container.register(ScopedValue, scope="task", from_context=True)
